...
def main():
    try:
        ...
        print(f"Data from file_reader():{data}")
        print()
        
        calculator = research.Calculations()
        
        heads, tails = calculator.counts(data)
        print(f"Counts from counts():{heads} {tails}")
        print()
    
        heads_frac, tails_frac = calculator.fractions(heads, tails)
        print(f"Fractions from fractions():{heads_frac:.4f} {tails_frac:.4f}")
        print()
        
        analytics = Analytics(data)
        
        random_predictions = analytics.predict_random(3)
        print(f"Predictions from predict_random(3): {random_predictions}")
        print()
        
        last_prediction = analytics.predict_last()
        print(f"Last prediction from predict_last:{last_prediction}")
